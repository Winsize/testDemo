...
function GameScene:init()
	...
	self:addNodeEventListener(cc.KEYPAD_EVENT , function(event)
		if event.key == "back" then
			display.replaceScene(require("app.scenes.SliderScene").new())
	...
	backbtn:onButtonClicked(function(event)
		...
		--暂停界面
		local layer = require("app.class.PauseLayer").new()
	...
	--触摸
	local isMove = true
	self._touchLayer = require("app.class.TouchLayer").new({
   ...
   --local contactListener  = cc.EventListenerPhysicsContact:create()
    ...
    local function onContactBegin(contact)
        ...
        if (node1:getTag()==601 and node2:getTag()==401) or (node1:getTag()==401 and node2:getTag()==601) then
			local loseLayer = require("app.class.LoseLayer").new()
